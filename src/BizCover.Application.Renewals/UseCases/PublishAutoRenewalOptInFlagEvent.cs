...
public interface IPublishAutoRenewalOptInFlagUpdatedEvent
{
    Task Publish(Guid expiringPolicyId, CancellationToken cancellationToken);
}

public class PublishAutoRenewalOptInFlagUpdatedEvent : IPublishAutoRenewalOptInFlagUpdatedEvent
{
    ...
    public PublishAutoRenewalOptInFlagUpdatedEvent(
    ...
    public async Task Publish(Guid expiringPolicyId, CancellationToken cancellationToken)
    {
        ...
        if (renewal != null)
        {
            await _queuePublisher.Publish(new AutoRenewalOptInFlagUpdatedEvent
            {
